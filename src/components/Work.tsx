...
/* ── Desktop card with cursor ripple + tilt ── */
function DesktopCard({ item, i }: { item: typeof work[0]; i: number }) {
  ...
  return (
    <motion.div variants={cardVariant} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-80px" }} transition={{ delay: i * 0.1 }} style={{ transformPerspective: 900 }}>
      <motion.div
      ...
      >
        {ripples.map(ripple => (
          <motion.div
            ...
            style={{ position: "absolute", left: ripple.x - 60, top: ripple.y - 60, width: 120, height: 120, borderRadius: "50%", border: `1.5px solid ${item.glow}`, pointerEvents: "none", zIndex: 10 }}
          />
        ))}
        <motion.div
          style={{ x: followerX, y: followerY, opacity: followerOpacity, border: `1.5px solid ${item.glow}` }}
...
/* ── Mobile card with touch ripple + press scale ── */
function MobileCard({ item, i }: { item: typeof work[0]; i: number }) {
  const [ripples, setRipples] = useState<Ripple[]>([]);
  ...
  function onTouchStart(e: React.TouchEvent<HTMLDivElement>) {
    ...
    // Emit 3 staggered rings per tap
  ...
  return (
    ...
    >
      <div
        ...
        className="relative rounded-2xl bg-[#0a0a0a] p-7 flex flex-col gap-5 overflow-hidden"
      >
        {ripples.map(ripple => (
          <motion.div
            key={ripple.id}
            ...
            style={{ position: "absolute", left: ripple.x - 70, top: ripple.y - 70, width: 140, height: 140, borderRadius: "50%", border: `1.5px solid ${item.glow}`, pointerEvents: "none", zIndex: 10 }}
          />
        ))}
        <CardContent item={item} />
      </div>
...
/* ── Shared card content ── */
function CardContent({ item }: { item: typeof work[0] }) {
  return (
    <>
...
export default function Work() {
  ...
  return (
    <section id="work" className="px-6 lg:px-16 py-28">
      <div className="mb-12">
        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="text-xs font-mono tracking-widest text-zinc-400 uppercase border border-zinc-200 rounded-full inline-flex px-4 py-1.5 mb-6">
          Experience
        </motion.p>
        <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.65, ease }} className="font-display font-extrabold tracking-[-0.03em] text-[#0a0a0a]" style={{ fontSize: "clamp(2.25rem, 5vw, 4.5rem)" }}>
          Where I&apos;ve worked.
        </motion.h2>
      </div>
      <div className="flex flex-col gap-4">
        {work.map((item, i) =>
          isMobile
            ? <MobileCard key={item.num} item={item} i={i} />
            : <DesktopCard key={item.num} item={item} i={i} />
        )}
      </div>
    </section>
  );
